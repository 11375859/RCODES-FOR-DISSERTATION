...
linear_pred_test <- predict(fit_cvd, newdata = test_set, type = "lp") + log(2) # 如果你之前的offset中用了log(2)

# 直接使用linear_pred_test计算expected values
...
# 错误模型1的线性预测值，需要手动计算
test_set$lp_bad1_sd <- predict(fit_cvd, newdata = test_set, type = "lp") + log(2)
test_set$p_bad1_sd <- exp(test_set$lp_bad1_sd)  # 使用手动计算的lp_bad1_sd计算expected
...
# 使用test_set数据集计算其他模型的线性预测器
test_set$pred_sd <- riskRegression::predictRisk(fit_cvd, newdata = test_set, times = 5)
test_set$pred_bad1_sd <- 1 - exp(-exp(test_set$lp_bad1_sd))  # 使用调整后的lp计算预测值
test_set$pred_bad3_sd <- riskRegression::predictRisk(fit_cvd_bad3, newdata = test_set, times = 5)
test_set$pred_bad2_sd <- riskRegression::predictRisk(fit_cvd_bad2, newdata = test_set, times = 5)
...
absdiff_cph_sd <- na.omit(absdiff_cph_sd)  # 移除可能存在的 NA 或 NaN 值
numsum_cph_sd <- c(
...
absdiff_cph_bad1_sd <- na.omit(absdiff_cph_bad1_sd)  # 移除可能存在的 NA 或 NaN 值
...
absdiff_cph_bad3_sd <- na.omit(absdiff_cph_bad3_sd)  # 移除可能存在的 NA 或 NaN 值
...
absdiff_cph_bad2_sd <- na.omit(absdiff_cph_bad2_sd)  # 移除可能存在的 NA 或 NaN 值
...
```{r}
# 定义一个bootstrap函数来计算ICI、E50和E90，处理NA值
...
# 为Original Model计算bootstrap的置信区间
...
# 为Error Model 1计算bootstrap的置信区间
...
# 为Error Model 3计算bootstrap的置信区间
...
# 为Error Model 2计算bootstrap的置信区间
...
time range 1
```{r}
# 使用test_set数据集计算Prognostic Index (PI)和expected number of events
...
# 计算expected number of events
exp_t_sd <- predict(fit_cvd, newdata = test_set, type = "expected")
exp_t_bad1_sd <- 1 - exp(-exp(test_set$PI_bad1_sd))  # 使用调整后的lp计算expected
...
time range 2
```{r}
# 使用test_set数据集计算Cumulative hazard和Prognostic Index (PI)
...
# 计算bad3和bad2的Cumulative hazard和PI
